...
//	define
//===================================================================================
#define LED_RED		RA0     //red LED (Active HIGH)
#define LED_YELLOW	RA1	//yellow LED (Active HIGH)

#define RESET		RC4	//Reset pin of the LCD display
#define ENABLE		RC5	//Enable pin of the LCD display
#define lcd_data	PORTB	//LCD 8-bit data port

#define BUTTON1		RA2	//Push button 1 (Active LOW)
#define BUTTON2 	RA3	//Push button 2 (Active LOW)
#define BUTTON3		RA4	//Push button 3 (Active LOW)
...
void send_config(unsigned char data);       //function to configure LCD display
void delay(unsigned long data);             //delay function
void send_char(unsigned char data);         //function to send character to LCD display
void lcd_goto(unsigned char data);          //function to set lcd cursor to desired location
void lcd_clr(void);                         //function to clear LCD display
void send_string(const char *s)	;           //function to send string to LCD display
void uart_send(unsigned char data);         //function to transmit data by data to target device
void send_cmd(unsigned char CMD, unsigned char P1, unsigned char P2, unsigned char P3); //function to send command
unsigned char uart_receive(void);           //function to receive data from uart buffer
void mode(unsigned char data);              //modes operation
void display_status(unsigned char type, unsigned char data0, unsigned char data);   //function to display the status of reply
void receive_reply(void);                   //function to get reply from UART
void receive_reply_nodisplay(void);         //function to get reply from UART but not display the status
void setting_repeat(void);                  //function to verify admin
void CheckAdmin(void);                      //function to verify admin
void AddNew(unsigned char data);            //add new fingerprint into database
void store_eeprom(unsigned char data,unsigned char location);   //store data into specified location in EEPROM
unsigned char get_eeprom(unsigned char location);               //access EEPROM to retrieve data
void view_attendance (void);                //function to display attendance
void init_attendance(void);                 //Initialize attendance list
void clear_attendance(void);                //clear attendance marking and reset to default
void upload_HT(void);                       //function to upload attendance to Hyper Terminal
void upload_GUI(void);                      //function to upload attendance to GUI
void allign(void);                          //function to allign the text in Hyper Terminal
void ResetAdmin(void);                      //function to reset and re-assign admin
...
void main(void)
{
	...
	if((!BUTTON1) && (!BUTTON2))		//if both SW1 and SW2 are pressed ->entering reset admin mode
	{
		delay(100000);
		if((!BUTTON1) && (!BUTTON2))	//wait for release
		{
...
{	
	while(usertype_enter==0)                        //usertype_enter flag not set
	{
		...
		if(!BUTTON1)				//User select Admin
		{
			while(!BUTTON1);//(!ButA);	//wait for release
		...
		else if (!BUTTON2)			//User select user
		{
			while(!BUTTON2);		//wait for release
	...
	{
		if((!BUTTON1))                                      //check whether SW1(INCREMENT) is pressed
		{		
		...
		else if((!BUTTON2))		//check whether SW2(DECREMENT) is pressed
		{		
			while((!BUTTON2));	//wait for release
		...
		else if(!BUTTON3)			//check whether SW3(CONFIRM) is pressed
		{
			while(!BUTTON3);                //wait for release
...
{
	RESET=0;            //set lcd to configuration mode
	lcd_data=data;      //lcd data port = data
	ENABLE=1;           //pulse enable to confirm the data
	delay(50);
	ENABLE=0;
	delay(50);
}
...
{
	RESET=1;				//set lcd to display mode
	lcd_data=data;				//lcd data port = data
	ENABLE=1;				//pulse enable to confirm the data
	delay(10);
	ENABLE=0;
...
{	
	...
	{
		if(!BUTTON1)               //check whether SW1 is pressed
		{		
			while(!BUTTON1);        //wait for release
		...
		else if(!BUTTON2)                   //check whether SW2 is pressed
		{		
			while(!BUTTON2);            //wait for release
		...
		else if(!BUTTON3)		//check whether SW3 is pressed
		{
			lcd_clr();		//clear lcd display
	...
	{	
		...
		if(!BUTTON1)				//if SW1 is pressed
			{	
				while(!BUTTON1);		//waiting for release
				if (location<5)			//if location is not reaching 5
				location++;			//increment by 1
				else
				location=1;			//reset back to 1
			}
		else if(!BUTTON2)			//if SW2 is pressed
		{		
			while(!BUTTON2);		//wait for release
		...
		else if(!BUTTON3)				//if SW3 is pressed
			{
				while(!BUTTON3);		//waiting for release
...
void display_status(unsigned char type, unsigned char data0, unsigned char data)	//show the status of the reply
{
	...
	switch(data)
	{
		case ACK_SUCCESS:		LED_YELLOW=1;								//turn ON yellow LED
								if((type != 1) && (type != 2))
		...
		case ACK_FAIL:			LED_RED=1;						//turn ON red LED
								BUZZER=1;			//ON buzzer
								lcd_goto(20);			//set the lcd cursor to location 20
		...
		case ACK_TIMEOUT:		LED_RED=1;						//turn ON red LED
								BUZZER=1;			//On buzzer
								lcd_goto(20);			//set the lcd cursor to location 20
		...
		case ACK_USER_EXIST:	LED_RED=1;                                                      //turn ON red LED
								BUZZER=1;                            //ON buzzer
								lcd_goto(20);                        //set the lcd cursor to location 20
		...
		case ACK_NOUSER:		LED_RED=1;                                             //turn ON red LED
								BUZZER=1;                           //ON buzzer
								lcd_goto(20);                       //set the lcd cursor to location 20
		...
		case ACK_ACCESS:		LED_YELLOW=1;						//turn ON yellow LED
		...
		default:				LED_RED=1;				//turn ON red LED
								LED_YELLOW=1;			//turn ON yellow LED
